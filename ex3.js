...
let nuevoArray = numeros.map(x => x - 1);
...
let pacientesVisitados = pacientes.map(p => { p.ultimaVisita = new Date().toLocaleString(); return p; }); // TODO
...
let pacientesAvisos = pacientes.map(p => { p.avisos = (p.ultimaVisitaPagada) ? "" : "Paciente moroso, cobrar!"; return p; }); // TODO
...
let pacientesLista = pacientes.map(p => `<li>${p.nombre}</li>`); // TODO
